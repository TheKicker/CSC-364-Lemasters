<?php
/**
 * Class to handle interaction with the products table in the database.
...
class orderProfile
{
     ...
     * @return mixed
     */
    public function getOrders($active = null)
    {
    ...
    {
        $sql = "update products set name = '" . $data['name'] . "', description = '" . $data['description'] . "', qty_available = '" . $data['qty_available'] . "', price = '" . $data['price'] . " where id = " . $data['id'] . ';';
        $result = $this->db->query($sql);
        return $result;
    }
    /**
    ...
    {
        // In User class this was...
        // $sql = 'update customers set isActive=0 where id = ' . $id . ';';
        // In products it is this instead...
